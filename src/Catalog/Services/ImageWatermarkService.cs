...
{
    //Não dá para usar um scoped aqui,  o backgroundService roda em singleton.
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    var unprocessedProducts = await context.Products.Where(_ => !_.MarkedImageWatermark).ToListAsync();
                    ...
                    foreach (var product in unprocessedProducts)
                    {
                        try
                        {
                            ...
                            var newimagePath = Path.Combine(_webHostEnvironment.WebRootPath, "images", "prod_" + product.Image);
                            ...
                            product.MarkedImageWatermark = true;
